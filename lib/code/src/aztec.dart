...
class BarcodeAztec extends Barcode2D {
  ...
  static void _init() {
    ...
    _charMap[_EncodingMode.upper] = List<int?>.filled(256, null);
    _charMap[_EncodingMode.lower] = List<int?>.filled(256, null);
    _charMap[_EncodingMode.digit] = List<int?>.filled(256, null);
    _charMap[_EncodingMode.mixed] = List<int?>.filled(256, null);
    _charMap[_EncodingMode.punct] = List<int?>.filled(256, null);

    _charMap[_EncodingMode.upper]![0x20] = 1;
    for (var c = 0x41; c <= 0x5a; c++) {
      _charMap[_EncodingMode.upper]![c] = c - 0x41 + 2;
    }

    _charMap[_EncodingMode.lower]![0x20] = 1;
    for (var c = 0x61; c <= 0x7a; c++) {
      _charMap[_EncodingMode.lower]![c] = c - 0x61 + 2;
    }
    _charMap[_EncodingMode.digit]![0x20] = 1;
    for (var c = 0x30; c <= 0x39; c++) {
      _charMap[_EncodingMode.digit]![c] = c - 0x30 + 2;
    }
    _charMap[_EncodingMode.digit]![0x2c] = 12;
    _charMap[_EncodingMode.digit]![0x2e] = 13;
    ...
    for (var i = 0; i < mixedTable.length; i++) {
      ...
      _charMap[_EncodingMode.mixed]![v] = i;
    }
    ...
    for (var i = 0; i < punctTable.length; i++) {
      ...
      if (v > 0) {
        _charMap[_EncodingMode.punct]![v] = i;
      }
    }
  }
  ...
  List<_State> _updateStateForPair(
    ...
    result.add(stateNoBinary.latchAndAppend(_EncodingMode.punct, pairCode));
    if (s.mode != _EncodingMode.punct) {
      // Possibility 2.  Shift to MODE_PUNCT, and then append this code.
      // Every state except MODE_PUNCT (handled above) can shift
      result.add(stateNoBinary.shiftAndAppend(_EncodingMode.punct, pairCode));
    }
    if (pairCode == 3 || pairCode == 4) {
      // both characters are in DIGITS.  Sometimes better to just add two digits
      final digitState = stateNoBinary
          .latchAndAppend(_EncodingMode.digit, 16 - pairCode)
          . // period or comma in DIGIT
          latchAndAppend(_EncodingMode.digit, 1); // space in DIGIT
...
const _shiftTable = <_EncodingMode, Map<_EncodingMode, int>>{
  _EncodingMode.upper: {_EncodingMode.punct: 0},
  _EncodingMode.lower: {_EncodingMode.punct: 0, _EncodingMode.upper: 28},
  _EncodingMode.mixed: {_EncodingMode.punct: 0},
  _EncodingMode.digit: {_EncodingMode.punct: 0, _EncodingMode.upper: 15},
};
...
class _State {
  ...
  static const latchTable = <_EncodingMode, Map<_EncodingMode, int>>{
    _EncodingMode.upper: {
      _EncodingMode.upper: 0,
      _EncodingMode.lower: (5 << 16) + 28,
      _EncodingMode.digit: (5 << 16) + 30,
      _EncodingMode.mixed: (5 << 16) + 29,
      _EncodingMode.punct: (10 << 16) + (29 << 5) + 30,
    },
    _EncodingMode.lower: {
      _EncodingMode.upper: (9 << 16) + (30 << 4) + 14,
      _EncodingMode.lower: 0,
      _EncodingMode.digit: (5 << 16) + 30,
      _EncodingMode.mixed: (5 << 16) + 29,
      _EncodingMode.punct: (10 << 16) + (29 << 5) + 30,
    },
    _EncodingMode.digit: {
      _EncodingMode.upper: (4 << 16) + 14,
      _EncodingMode.lower: (9 << 16) + (14 << 5) + 28,
      _EncodingMode.digit: 0,
      _EncodingMode.mixed: (9 << 16) + (14 << 5) + 29,
      _EncodingMode.punct: (14 << 16) + (14 << 10) + (29 << 5) + 30,
    },
    _EncodingMode.mixed: {
      _EncodingMode.upper: (5 << 16) + 29,
      _EncodingMode.lower: (5 << 16) + 28,
      _EncodingMode.digit: (10 << 16) + (29 << 5) + 30,
      _EncodingMode.mixed: 0,
      _EncodingMode.punct: (5 << 16) + 30,
    },
    _EncodingMode.punct: {
      _EncodingMode.upper: (5 << 16) + 31,
      _EncodingMode.lower: (10 << 16) + (31 << 5) + 28,
      _EncodingMode.digit: (10 << 16) + (31 << 5) + 30,
      _EncodingMode.mixed: (10 << 16) + (31 << 5) + 29,
      _EncodingMode.punct: 0,
    },
  };
  ...
  _State addBinaryShiftChar(int index) {
    ...
    var mode = this.mode;
    var bitCnt = bitCount;
    if (this.mode == _EncodingMode.punct || this.mode == _EncodingMode.digit) {
      final latch = latchTable[this.mode]![_EncodingMode.upper]!;
      tokens = _SimpleToken(tokens, latch & 0xFFFF, latch >> 16);
      bitCnt += latch >> 16;
      mode = _EncodingMode.upper;
    }
